...
from generate_grants import (to_date, remove_comments, get_board_member_name,
                             get_votes)
...
def board_history():
    return [  # sorted!
        {'from': datetime(2020, 1, 1), 'members': {'alice': 'Alice',
                                                   'doubravka': 'Doubravka'}},
...
@pytest.mark.parametrize('username,voted_at,expected', [
...
def test_get_votes_understands_all_relevant_emojis(content, board_history,
                                                   expected):
    reactions = [
